...
function generateChannelArtifacts() {
  ...
  if [ "$DEV_MODE" = false ] ; then
    echo
    echo "#####################################################################"
    echo "#######  Generating anchor peer update for ProviderOrgMSP  ##########"
    echo "#####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/ProviderOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ProviderOrgMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for ProviderOrgMSP..."
      exit 1
    fi
    ...
    ./channel-artifacts/UserOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg UserOrgMSP -channelID $CHANNEL_NAME
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for UserOrgMSP..."
      exit 1
    fi

    echo
    echo "####################################################################"
    echo "#######  Generating anchor peer update for RepairerOrgMSP  ##########"
    echo "####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/RepairerOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg RepairerOrgMSP -channelID $CHANNEL_NAME
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for RepairerOrgMSP..."
      exit 1
    fi
    echo
  fi
